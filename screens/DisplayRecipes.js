...
export default function DisplayRecipes({ route, navigation }) {
  ...
  const { ingredientsQuery, allergies } = route.params || {};
...
export default function DisplayRecipes({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        // Construct the query URL including ingredients and allergies
        let queryUrl = `http://localhost:5000/recipes?ingredients=${ingredientsQuery}`;
        if (allergies && allergies.length > 0) {
          const allergiesQuery = allergies.join(',');
          queryUrl += `&allergies=${allergiesQuery}`;
        }
